 ...
 *
 * Visual indicator for sync/offline status.
 * Shows:
 * - Online/offline status
 * - Pending sync count
 * - Failed sync count with retry option
 * - Last sync time
 */
...
function StatusDot({ isOnline }) {
  return (
    <span
      className={`inline-block h-2 w-2 rounded-full ${isOnline ? 'bg-green-500' : 'bg-red-500'}`}
      aria-hidden="true"
    />
  )
}
  ...
  return (
    <div className="flex items-center gap-3 text-sm">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className={isOnline ? 'text-gray-600' : 'text-red-600'}>
